# Migrating Expo UI to Nextera Design System

Read `artifacts/nextera-design-system/docs/AGENTS.md` and
`artifacts/nextera-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/nextera-design-system/lib/native-theme` and
  `@workspace/nextera-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/nextera-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/nextera-design-system` and retains only
